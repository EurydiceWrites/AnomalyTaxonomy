...
3. **Stage 1B: Conformance reproduction** — Walk-and-inherit episode segmentation tested across 4 iterations (v1–v4). Best result: v4 at 56.8% overall, 63.1% orthodox (DEC-012: A115/A116/A220/A221 excluded as noisy anchors). Conference (C/M) cannot be anchored — null result. **Major finding:** family distribution peaks independently recover Bullard's episode sequence (E→B→X→C→M→A maps to Capture→Beings→Examination→Conference→Messages→Aftermath). Full independence test requires non-Bullard coded data.
